import { c as create_ssr_component, f as each, e as escape, d as add_attribute } from "./index.js";
...
const PostsList = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<ul class="${"posts-list"}">${each(posts, (post) => {
    return `<li><article><a href="${"/blog/" + escape(post.slug, true)}"><img${add_attribute("src", post.coverImage, 0)} alt="${""}"${add_attribute("width", post.coverWidth, 0)}${add_attribute("height", post.coverHeight, 0)} style="${"ratio: " + escape(post.coverWidth, true) + " / " + escape(post.coverHeight, true)}">
